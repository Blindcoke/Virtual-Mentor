...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(`/profile?calendar_error=${encodeURIComponent(error)}`, request.url)
    );
  }

  if (!code) {
    return NextResponse.redirect(
      new URL('/profile?calendar_error=no_code', request.url)
    );
  }
  ...
  try {
    ...
    // Store tokens in httpOnly cookies (temporary solution)
    ...
    return NextResponse.redirect(
      new URL('/profile?calendar_error=oauth_failed', request.url)
    );
  }
}
